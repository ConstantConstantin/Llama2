...
@testset "tokenizer" begin
    ...
    @testset "isless(::TokenIndex, ::TokenIndex)" begin
            # Comparison helper for sorting by string (and/or tie-breaking rules).
            @test isless(Llama2.TokenIndex("A", 1), Llama2.TokenIndex("B", 2)) == true
            @test isless(Llama2.TokenIndex("B", 1), Llama2.TokenIndex("A", 2)) == false

            # Same string should not be considered "less".
            @test isless(Llama2.TokenIndex("aa", 1), Llama2.TokenIndex("aa", 2)) == false
        end
